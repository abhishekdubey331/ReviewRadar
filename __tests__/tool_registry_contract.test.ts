...
describe("tool registry contract", () => {
    ...
    it("allows omitted source for analyze and critical alerts tools (defaults to local dataset)", () => {
        const analyze = getTool("reviews_analyze");
        const safety = getTool("reviews_get_critical_alerts");
        expect((analyze.inputSchema as any).required).toBeUndefined();
        expect((safety.inputSchema as any).required).toBeUndefined();
    });

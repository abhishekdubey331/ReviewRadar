...
import { getCriticalAlertsTool } from '../src/tools/safety_alerts.js';

describe('reviews.get_critical_alerts tool', () => {
    it('returns only safety alerts and metadata using the fast-path', async () => {
        ...
        const result = await getCriticalAlertsTool(input, mockVectorStore);
    ...
    it('caps returned alerts using alert_limit', async () => {
        const input = {
            source: {
                type: 'inline',
                reviews: [
                    { review_id: "2", content: "SOS not working one", score: 1 },
                    { review_id: "3", content: "SOS not working two", score: 1 },
                    { review_id: "4", content: "App freezes often", score: 2 }
                ]
            },
            options: { alert_limit: 1 }
        };
        ...
        const result = await getCriticalAlertsTool(input, mockVectorStore);
        expect(result.data.metadata.total_alerts).toBeGreaterThan(1);
        expect(result.data.metadata.returned_alerts).toBe(1);
        expect(result.data.safety_alerts).toHaveLength(1);
    });
});

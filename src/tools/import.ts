...
export async function loadReviews(input: unknown) {
    ...
    for (const raw of rawReviews) {
        ...
        // Allow search-result-shaped records to be reused as analyze/import input.
        if (!normalized.review_id && typeof normalized.id === "string") {
            normalized.review_id = normalized.id;
        }
        if (!normalized.review_created_at && typeof normalized.date === "string") {
            normalized.review_created_at = normalized.date;
        }
        if (!normalized.user_name && typeof normalized.author === "string") {
            normalized.user_name = normalized.author;
        }
